...
- [ ] **Step 1: `config.toml`의 `[auth]` 확인·수정**
...
- [ ] **Step 2: `[auth.external.google]` 켜기**
...
- [ ] **Step 3: `.env`에 Google 자격값 넣기** (사용자, 커밋 금지)
...
- [ ] **Step 4: 커밋** (config.toml만, `.env`는 제외)
...
- [ ] **Step 3: `.env.local`을 로컬 스택으로 교체** (사용자)
