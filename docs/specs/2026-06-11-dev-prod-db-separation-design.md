...
> **용어**: **마이그레이션**(migration) = DB 구조 변경을 차곡차곡 쌓는 "변경 기록부"(파일 하나 = 변경 한 단계). **baseline**(베이스라인) = 그중 현재 스키마를 통째로 담은 _맨 첫 번째_ 파일(출발점). **provision**(프로비저닝) = 빈 DB에 표·정책·함수를 실제로 만들어 "쓸 수 있는 상태로 갖추는" 것.
