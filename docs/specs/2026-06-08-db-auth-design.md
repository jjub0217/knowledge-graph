...
PUT  → 세션 확인 → 내 기존 행을 비우고 본문의 그래프로 교체(통째 저장)
```

- 그래프 통째 교체(기존 `saveGraph(전체)` 흐름 그대로). 부분 갱신보다 단순하고, 그래프 규모가 작아 비용도 작다.
- 라우트는 **로그인 사용자의 세션 토큰으로** Supabase를 호출 → RLS가 그 사람 기준으로 자동 적용. 만능 키(service_role) 불필요.

### 4.5 행 ↔ 그래프 변환 (순수 함수, TDD)

- `graphToRows(graph, userId)` → `{ nodeRows, edgeRows }`
- `rowsToGraph(nodeRows, edgeRows)` → `Graph`
...
그래프 변경 시
   ...
   └─ 로그인  → 디바운스 후 PUT /api/graph → 서버에 통째 저장
```
...
| 대상 | 방법 |
| --- | --- |
| `graphToRows` / `rowsToGraph` (순수 함수) | **TDD**(셋째 사이클) |
