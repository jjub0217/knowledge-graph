...
| `.env.local` / Vercel 환경변수 | 신규 | `NEXT_PUBLIC_SUPABASE_URL` · `NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY` |
...
- `NEXT_PUBLIC_SUPABASE_URL`·`NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY`는 **공개용**(브라우저가 쓰라고 만든 값) — 비밀 아님. `.env.local` + Vercel에 등록.
