...
## 동작 확인
- 스크린샷/GIF (UI 변경 시 — 자동 테스트가 못 덮는 시각적 확인)
- 수동 검증: [예: 로그 업로드 → 후보 채택 → 점 표시 확인]
...
- **한 이슈를 여러 PR로 분할**할 때는 **마지막 PR에만** `Closes #N`을 붙인다(분할 PR마다 붙이면 첫 머지에 이슈가 닫혀버림). 마지막 PR에도 못 붙였으면 **머지 후 이슈를 수동으로 닫는다**. (예: #38은 7개로 쪼개며 아무 PR도 `Closes`를 안 붙여 열린 채 남았었음)
...
- **계획서 체크박스 동기화**: Task·Step을 완료하면 그때그때 계획서(`docs/plans/…`)의 `- [ ]`를 `- [x]`로 체크한다. PR을 올리거나 머지될 때 함께 갱신 — 진행 상황과 문서가 어긋나지 않게. (2026-06-05 누락을 겪고 추가한 규칙)
